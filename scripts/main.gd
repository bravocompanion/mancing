...
var village_npcs := {"darto":Vector2(180,410),"yati":Vector2(330,410),"rian":Vector2(500,410),"tika":Vector2(405,520),"ujang":Vector2(590,590),"beni":Vector2(620,350)}
var swamp_npcs := {"maman":Vector2(190,430),"sari":Vector2(520,500)}
var fishing_spots := {"village":[Vector2(100,940),Vector2(280,960),Vector2(500,945),Vector2(640,965)],"swamp":[Vector2(90,900),Vector2(290,930),Vector2(500,910),Vector2(635,940)]}
...
func build_ui() -> void:
    ...
    add_child(ui)
    hud=Label.new(); hud.position=Vector2(22,18); hud.size=Vector2(676,155); hud.add_theme_font_size_override("font_size",22); ui.add_child(hud)
    hint_label=Label.new(); hint_label.position=Vector2(22,1080); hint_label.size=Vector2(676,48); hint_label.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER; hint_label.add_theme_font_size_override("font_size",18); ui.add_child(hint_label)
    tension_label=Label.new(); tension_label.position=Vector2(60,880); tension_label.size=Vector2(600,120); tension_label.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER; tension_label.add_theme_font_size_override("font_size",22); ui.add_child(tension_label)
    dialog_panel=Panel.new(); dialog_panel.position=Vector2(45,250); dialog_panel.size=Vector2(630,470); dialog_panel.visible=false; ui.add_child(dialog_panel)
    var box:=VBoxContainer.new(); box.position=Vector2(24,22); box.size=Vector2(582,425); dialog_panel.add_child(box)
    dialog_title=Label.new(); dialog_title.add_theme_font_size_override("font_size",25); box.add_child(dialog_title)
    dialog_body=Label.new(); dialog_body.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; dialog_body.custom_minimum_size=Vector2(570,260); dialog_body.add_theme_font_size_override("font_size",20); box.add_child(dialog_body)
    dialog_actions=HBoxContainer.new(); dialog_actions.add_theme_constant_override("separation",10); box.add_child(dialog_actions)
    status_label=Label.new(); status_label.position=Vector2(40,220); status_label.size=Vector2(640,700); status_label.visible=false; status_label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; status_label.add_theme_font_size_override("font_size",22); ui.add_child(status_label)
    make_touch_button(ui,"up","▲",Vector2(95,1080),Vector2(78,70)); make_touch_button(ui,"left","◀",Vector2(15,1155),Vector2(78,70)); make_touch_button(ui,"down","▼",Vector2(95,1155),Vector2(78,70)); make_touch_button(ui,"right","▶",Vector2(175,1155),Vector2(78,70)); make_touch_button(ui,"interact","E",Vector2(455,1150),Vector2(72,76)); make_touch_button(ui,"fish","MANCING",Vector2(535,1135),Vector2(165,92)); make_touch_button(ui,"status","STATUS",Vector2(535,1035),Vector2(165,60))

func make_touch_button(parent:CanvasLayer,key:String,caption:String,pos:Vector2,size:Vector2)->void:
    var b:=Button.new(); b.text=caption; b.position=pos; b.size=size; b.add_theme_font_size_override("font_size",18); parent.add_child(b); touch_buttons[key]=b
    if key in ["up","down","left","right"]:
        b.button_down.connect(func(): set_touch_move(key,true)); b.button_up.connect(func(): set_touch_move(key,false))
    elif key=="fish":
        b.button_down.connect(func(): fish_button_held=true; on_fish_pressed()); b.button_up.connect(func(): fish_button_held=false)
...
func _process(delta:float)->void:
    ...
    update_fishing(delta); update_hud(); queue_redraw()
...
func update_player(delta:float)->void:
    ...
    var next:=player+dir*speed*delta; next.x=clamp(next.x,28.0,W-28.0); next.y=clamp(next.y,190.0,1015.0)
    var water_y:=890.0 if zone=="swamp" else 920.0; var bridge_x:=Vector2(315,405) if zone=="village" else Vector2(325,395)
    if next.y>water_y and not(next.x>bridge_x.x and next.x<bridge_x.y):next.y=water_y
    player=next

func update_time(delta:float)->void:
    state.minute+=delta*.42
    if state.minute>=1440.0:state.day+=1;state.minute=375.0;state.stamina=state.max_stamina;roll_weather();save_game()

func roll_weather()->void:
    var r:=rng.randf(); state.weather="Cerah" if r<.48 else("Mendung" if r<.76 else("Hujan" if r<.94 else "Badai"))
...
func _draw()->void:
    draw_rect(Rect2(0,0,W,H),Color("6c9658") if zone=="village" else Color("637b50")); draw_rect(Rect2(0,610,W,80),Color("b3a074") if zone=="village" else Color("8c825f")); draw_rect(Rect2(330,170,60,750),Color("b3a074") if zone=="village" else Color("8c825f"))
    var wy:=920.0 if zone=="village" else 890.0; draw_rect(Rect2(0,wy,W,H-wy),Color("3f8da8") if zone=="village" else Color("477b70"))
    for x in range(20,700,80):draw_line(Vector2(x,wy+35+(x%3)*8),Vector2(x+38,wy+35+(x%3)*8),Color(1,1,1,.18),2)
    var bx:=315.0 if zone=="village" else 325.0; draw_rect(Rect2(bx,wy-12,90,100),Color("9d7650"))
    if zone=="village":draw_building(Rect2(45,260,130,105),"Rumah");draw_building(Rect2(235,260,130,105),"Warung");draw_building(Rect2(420,260,130,105),"Toko");draw_building(Rect2(565,240,115,105),"Balai")
    else:draw_building(Rect2(50,270,130,100),"Pondok");draw_building(Rect2(500,260,145,100),"Nipah")
    for p in [Vector2(40,470),Vector2(110,760),Vector2(650,470),Vector2(620,790),Vector2(250,500)]:draw_circle(p,26,Color("4f7847"));draw_circle(p+Vector2(-10,-8),15,Color("658e55"))
    for s in fishing_spots[zone]:draw_arc(s,18,0,TAU,28,Color(1,1,1,.75),2);draw_arc(s+Vector2(5,4),10,0,TAU,20,Color(1,1,1,.45),2)
    ...
    var gx:=685.0 if zone=="village" else 8.0;draw_rect(Rect2(gx,600,27,100),Color("67563d"));draw_circle(player,14,Color("efbd78"));draw_rect(Rect2(player.x-12,player.y+12,24,30),Color("35617c"))
...
func draw_building(r:Rect2,label:String)->void:
    draw_rect(r,Color("d0bea0"));var roof:=PackedVector2Array([Vector2(r.position.x-8,r.position.y+18),Vector2(r.position.x+r.size.x/2,r.position.y-24),Vector2(r.end.x+8,r.position.y+18)]);draw_colored_polygon(roof,Color("7d4f3e"));draw_string(ThemeDB.fallback_font,r.position+Vector2(15,r.size.y+23),label,HORIZONTAL_ALIGNMENT_LEFT,110,17,Color("20352a"))

func nearest_npc()->String:
    var npcs:=village_npcs if zone=="village" else swamp_npcs;var best:="";var best_d:=9999.0
    for id in npcs:
...
func near_spot()->bool:
    for s in fishing_spots[zone]:
        if player.distance_to(s)<75.0:return true
    return false
...
func talk_npc(id:String)->void:
    ...
    if id=="darto":
        if not state.quest.nila_accepted:show_dialog("Pak Darto — Optimistis",banter+"\n\nSaya butuh 3 nila buat acara RT. Bantu?",[{"text":"Gas, Pak","call":func():state.quest.nila_accepted=true;save_game();close_dialog()},{"text":"Nanti","call":close_dialog}]);return
        if not state.quest.nila_done:
            var have:=int(state.inventory.get("nila",0))
            if have>=3:show_dialog("Pak Darto — Optimistis",banter+"\n\nNila sudah lengkap. Saya ambil 3 ekor ya. Jalan rawa saya bukakan.",[{"text":"Serahkan","call":func():state.inventory.nila=have-3;state.money+=600;state.quest.nila_done=true;state.quest.swamp_unlocked=true;gain_xp(50);save_game();close_dialog()},{"text":"Nanti","call":close_dialog}]);return
            show_dialog("Pak Darto — Optimistis",banter+"\n\nNila baru %d/3."%have,[{"text":"Oke","call":close_dialog}]);return
    elif id=="yati":
        var value:=inventory_value()
        if value>0:show_dialog("Bu Yati — Pedagang",banter+"\n\nSemua ikanmu saya beli Rp%s."%format_money(value),[{"text":"Jual semua","call":func():state.money+=value;state.inventory={};save_game();close_dialog()},{"text":"Simpan","call":close_dialog}]);return
    elif id=="rian":show_dialog("Bang Rian — Teknisi",banter+"\n\nCacing 5 ekor Rp100. Upgrade alat menyusul di build berikutnya.",[{"text":"Beli 5 cacing","call":func():buy_bait()},{"text":"Tutup","call":close_dialog}]);return
    elif id=="tika":show_dialog("Mbak Tika — Jamu",banter+"\n\nKopi Rp120 (+35 tenaga). Jamu Rp180 (hoki 3 lemparan).",[{"text":"Kopi","call":func():buy_item("coffee")},{"text":"Jamu","call":func():buy_item("jamu")},{"text":"Tutup","call":close_dialog}]);return
    elif id=="beni":talk_beni(banter);return
    show_dialog(npc_profiles[id].name+" — "+npc_profiles[id].trait,banter,[{"text":"Oke","call":close_dialog}])

func talk_beni(banter:String)->void:
    if state.side_quest.beni_done:show_dialog("Pak Beni — Mantan Pemancing Lomba",banter+"\n\nRekormu sudah lumayan. Jangan besar kepala.",[{"text":"Oke","call":close_dialog}]);return
    if state.side_quest.beni_ready:show_dialog("Pak Beni — Mantan Pemancing Lomba",banter+"\n\nRekor %s cm saya akui. Sedikit."%str(state.stats.biggest),[{"text":"Ambil hadiah","call":func():state.side_quest.beni_done=true;state.money+=900;state.items.jamu+=1;gain_xp(90);save_game();close_dialog()}]);return
    if not state.side_quest.beni_accepted:show_dialog("Pak Beni — Mantan Pemancing Lomba",banter+"\n\nTangkap ikan minimal 70 cm. Biar saya tahu kamu bukan cuma jago foto.",[{"text":"Terima","call":func():state.side_quest.beni_accepted=true;save_game();close_dialog()},{"text":"Nanti","call":close_dialog}]);return
    show_dialog("Pak Beni — Mantan Pemancing Lomba",banter+"\n\nTarget 70 cm. Rekormu %s cm."%str(state.stats.biggest),[{"text":"Oke","call":close_dialog}])
...
func close_dialog()->void:dialog_open=false;dialog_panel.visible=false

func use_gate()->void:
    if zone=="village":
        if not state.quest.swamp_unlocked:show_dialog("Jalan Kedung Wungu","Masih ditutup warga. Selesaikan urusan Pak Darto dulu.",[{"text":"Oke","call":close_dialog}]);return
        zone="swamp";player=Vector2(80,650)
    else:zone="village";player=Vector2(625,650)
    save_game()
...
func on_fish_pressed()->void:
    ...
    if not fishing.is_empty():
        ...
        return
    if not near_spot() or state.bait<=0 or state.stamina<5:return
    state.bait-=1;state.stamina-=4;var event:="normal";var r:=rng.randf()
...
func hook_fish()->void:fishing.phase="fight";fishing.fish=roll_fish();fishing.tension=34.0;fishing.progress=0.0

func roll_fish()->String:
    var pool:Array=[];var night:=state.minute>=1140 or state.minute<300
    for id in fish_data:
        var f:Dictionary=fish_data[id]
        if not zone in f.zones:continue
        if f.get("night",false) and not night:continue
        pool.append(id)
    var rarity_roll:=rng.randf()+state.upgrades.hook*.025+(0.09 if state.buffs.luck_casts>0 else 0.0)+(0.10 if fishing.event=="feeding" else 0.0);var filtered:Array=[]
    for id in pool:
        var rr:String=fish_data[id].rarity
        if rr=="Langka" and rarity_roll>.91:filtered.append(id)
        elif rr=="Jarang" and rarity_roll>.60:filtered.append(id)
        elif rr=="Umum":filtered.append(id)
    if filtered.is_empty():filtered=pool
...
func update_fishing(delta:float)->void:
    ...
    if fishing.phase=="wait":
        fishing.timer-=delta;tension_label.text="Menunggu ikan..."
        if fishing.timer<=0:fishing.phase="bite";fishing.timer=.95
    elif fishing.phase=="bite":
        fishing.timer-=delta;tension_label.text="NYANGKUT! Tekan MANCING!"
        if fishing.timer<=0:fail_fish("Telat. Ikannya kabur.")
    elif fishing.phase=="fight":
        var f:Dictionary=fish_data[fishing.fish];var diff:float=min(.98,float(f.diff)+(0.11 if fishing.event=="current" else 0.0));fishing.burst_timer-=delta
        if fishing.burst_timer<=0 and fishing.burst<=0:fishing.burst=.45+diff*.55;fishing.burst_timer=rng.randf_range(1.8,max(2.0,3.0-diff))
        if fishing.burst>0:fishing.burst-=delta
        var burst_force:float=(28+diff*55)*float(f.burst) if fishing.burst>0 else 0.0;var gear_power:=1.0+state.upgrades.rod*.10+state.upgrades.line*.085
        if fish_button_held or Input.is_action_pressed("fish"):
            fishing.tension+=delta*((34+diff*37+burst_force)/(1+state.upgrades.line*.12));var sweet:=1.22 if fishing.tension>=42 and fishing.tension<=72 else .82;fishing.progress+=delta*((14+22*(1-diff))*gear_power*sweet*float(f.reel))
        else:fishing.tension-=delta*(48-7*diff);fishing.progress-=delta*(3+diff*5+(6 if fishing.burst>0 else 0))*float(f.loss)
        fishing.tension=clamp(fishing.tension,0.0,110.0);fishing.progress=clamp(fishing.progress,0.0,100.0)
        if fishing.tension>=100:fail_fish("KRAK! Senar putus.");return
        if fishing.tension<=2:
            fishing.escape+=delta
            if fishing.escape>1.05*float(f.escape):fail_fish("Senar terlalu kendur. Ikan lepas.");return
        else:fishing.escape=max(0.0,fishing.escape-delta*2)
        if fishing.progress>=100:catch_fish(fishing.fish);return
        tension_label.text="%s • %s%s\nTension %d%% | Tarikan %d%%"%[f.name,f.trait," • NGAMUK!" if fishing.burst>0 else "",int(fishing.tension),int(fishing.progress)]

func catch_fish(id:String)->void:
    var f:Dictionary=fish_data[id];var size:=snappedf(rng.randf_range(f.min,f.max),0.1);state.inventory[id]=int(state.inventory.get(id,0))+1;state.caught[id]=max(float(state.caught.get(id,0.0)),size);state.stats.fish_caught+=1;state.stats.biggest=max(float(state.stats.biggest),size)
    if f.rarity=="Langka":state.stats.rare_caught+=1
    var xp:=int((28 if f.rarity=="Langka" else(17 if f.rarity=="Jarang" else 9))+f.diff*12);gain_xp(xp)
    ...
    state.minute+=16;state.stamina=max(0.0,state.stamina-2);fishing={};tension_label.text="Dapat %s %.1f cm! +%d XP"%[f.name,size,xp];save_game();await get_tree().create_timer(2.0).timeout
    if fishing.is_empty():tension_label.text=""

func fail_fish(msg:String)->void:
    fishing={};tension_label.text=msg;state.minute+=7;save_game();await get_tree().create_timer(1.7).timeout
    if fishing.is_empty():tension_label.text=""

func inventory_value()->int:
    var total:=0
    for id in state.inventory:
        if fish_data.has(id):total+=int(state.inventory[id])*int(fish_data[id].price)
    return total

func gain_xp(amount:int)->void:
    state.xp+=amount
    while state.xp>=xp_need(state.level):state.xp-=xp_need(state.level);state.level+=1;state.max_stamina+=4;state.stamina=min(state.max_stamina,state.stamina+20)
...
func save_game()->void:
    var data:=state.duplicate(true);data["zone"]=zone;data["player"]=[player.x,player.y];var f:=FileAccess.open(SAVE_PATH,FileAccess.WRITE)
...
func load_game()->void:
    ...
    merge_state(state,parsed);zone=parsed.get("zone","village");var p=parsed.get("player",[360,700])
    if p is Array and p.size()>=2:player=Vector2(float(p[0]),float(p[1]))
